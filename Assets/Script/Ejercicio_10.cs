...
y deberemos setear una  variable booleana llamada estaAbierto en true o false, según corresponda, e informar por  pantalla mediante la misma si el estacionamiento se encuentra abierto o no. 
...
{
    public int horario;
    public int Hora_de_Apertura = 10;
    public int Hora_de_Cierre = 10;
    bool Abierto;
    // Start is called before the first frame update
    void Start()
    {
        if (horario >= Hora_de_Apertura && Hora_de_Apertura >= Hora_de_Cierre)
        {
            Abierto = true;
            Debug.Log("El estacionamiento esta abierto");
        }
        else if (horario < Hora_de_Apertura && horario >= 0)
        {
            Abierto = false;
            Debug.Log("El estacionamiento esta cerrado");
        }
        else if (horario < Hora_de_Apertura && horario <= 24)
        {
            Abierto = true;
            Debug.Log("El estacionamiento esta cerrado");
        }
